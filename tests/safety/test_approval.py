...
async def test_rejected_approval_blocks_the_action(analysed_run):
    analysed_run.request_approval("scn_recover_tonnes", actor_roles=["SHIFT_BOSS"])
    analysed_run.decide_approval(approve=False, approver_id="u", approver_role="SHIFT_BOSS")
...
async def test_approver_must_hold_a_required_role(analysed_run):
    analysed_run.request_approval("scn_preserve_equipment", actor_roles=["SHIFT_BOSS", "MAINTENANCE_PLANNER"])
    with pytest.raises(ActionRejected) as excinfo:
...
async def test_forged_token_is_refused(analysed_run):
    ...
    with pytest.raises(ActionRejected) as excinfo:
        analysed_run.gateway.execute(
            request, approval=analysed_run.approval, actor_roles=["SHIFT_BOSS"], evidence=[]
        )
    assert excinfo.value.code == "APPROVAL_INVALID"
...
async def test_action_outside_approved_scope_is_refused(analysed_run):
    ...
    analysed_run.request_approval("scn_protect_safety", actor_roles=["SHIFT_BOSS"])
    approval = analysed_run.decide_approval(approve=True, approver_id="u", approver_role="SHIFT_BOSS")
